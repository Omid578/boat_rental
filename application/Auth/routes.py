...
def Register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if user:
            ...
            user = Users(email=form.email.data,password=generate_password_hash(form.password.data),firstname=form.firstname.data,lastname=form.lastname.data,age=form.age.data,gender=form.gender.data,street=form.street.data,plz=form.plz.data,state=form.state.data,is_admin=0)
            db.session.add(user)
            db.session.commit()
